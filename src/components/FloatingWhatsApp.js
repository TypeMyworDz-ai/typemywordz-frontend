...
const FloatingWhatsApp = () => {
  ...
  const openLabel = () => setExpanded(true);
  const closeLabel = () => setExpanded(false);
  ...
  return (
    <a
      ...
      style={{
        ...
        padding: expanded ? '8px 14px 8px 10px' : '8px',
        overflow: 'hidden',
        borderRadius: '999px',
        background: '#25D366',
        color: '#fff',
        ...
        transition: 'max-width 180ms ease, padding 180ms ease, gap 180ms ease, transform 180ms ease',
    ...
    >
      ...
      <span style={{ opacity: expanded ? 1 : 0, transition: 'opacity 140ms ease', font: '600 13px var(--tm-font, Arial, sans-serif)' }}>
        Chat with us
      </span>
    </a>
  );
