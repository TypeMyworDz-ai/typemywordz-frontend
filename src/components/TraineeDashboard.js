...
export default function TraineeDashboard({ onBack, onOpenWork, showMessage }) {
  ...
  if (!enrolled && !worker) return <section className="tm-human-page"><button type="button" className="tm-human-back" onClick={onBack}>← Back to workspace</button><div className="tm-human-card"><h1>Training Room is locked</h1><p>Complete the $30 training enrollment first. Your registration can remain pending until checkout is completed.</p></div></section>;
  ...
  return <section className="tm-human-page" aria-labelledby="trainee-title">
    ...
    <div className="tm-human-intro"><div><h1 id="trainee-title">Your proofreading training</h1><p>You are enrolled. Work through each module and practical carefully; successful completion creates a chance to be considered for paid TypeMyworDz proofreading work.</p></div></div>
