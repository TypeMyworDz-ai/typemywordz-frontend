...
// This is the correct placement for a static, unchanging string.
...
const AdminAIFormatter = ({ showMessage }) => {
    ...
    try {
      const response = await fetch(`${RAILWAY_BACKEND_URL}/ai/admin-format`, {
        ...
        body: JSON.stringify({
          ...
          model: 'claude-3-sonnet-20240229', // Admin might use a more powerful model, or you can make it configurable
  ...
  }, [transcriptInput, formattingInstructions, showMessage, RAILWAY_BACKEND_URL]);
    ...
    <div style={{ padding: '20px', maxWidth: '1000px', margin: '0 auto', backgroundColor: '#f8f9fa', borderRadius: '15px', boxShadow: '0 10px 30px rgba(0,0,0,0.1)', marginTop: '20px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '15px', marginBottom: '30px', flexWrap: 'wrap' }}>
        <button
          ...
          disabled={!transcriptInput || !formattingInstructions || aiLoading}
          style={{
            padding: '12px 25px',
            backgroundColor: (!transcriptInput || !formattingInstructions || aiLoading) ? '#6c757d' : '#dc3545',
            color: 'white',
            border: 'none',
            borderRadius: '25px',
            cursor: (!transcriptInput || !formattingInstructions || aiLoading) ? 'not-allowed' : 'pointer',
            fontSize: '1rem',
            fontWeight: 'bold',
