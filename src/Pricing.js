        ...
        <div className="tm-pr-bundles">
          ...
          {customTopup && (
            <div className="tm-pr-bundle tm-pr-bundle-custom">
              ...
              <button type="button" className="tm-pr-buy" disabled={!isSignedIn || busy === `topup-custom-${customAmount}`} onClick={() => buy(`topup-custom-${customAmount}`)}>
                {buyLabel(`topup-custom-${customAmount}`, 'Buy this amount')}
              </button>
              <small>Minimum {customMinimum} credits. If you enter less, we’ll let you know before checkout.</small>
            </div>
          )}
        </div>
